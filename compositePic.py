...
def video_to_compound_pic(foreground_video, background_video):
    ...
    def code2():
        ...
        file_names = [
            config.foreground_pic_raw_1_path + "/" + item for item in file_names_1
        ]
    ...
    for i in range(png_count):
        background_pic = Image.open(
            config.background_pic_path + f"/frame_{str(i).zfill(4)}.png"
        )
        foreground_pic = Image.open(
            config.foreground_pic_path + f"/frame_{str(i).zfill(4)}.png"
        )
        # 将要合成的图片粘贴到背景图片上
